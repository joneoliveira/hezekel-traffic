...
function computeStatus(score: number, reach: number, avgReach: number): string {
  if (reach < avgReach * 0.1) return 'Learning';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const processed: OrganicPost[] = raw.map(({ row, reach, views, likes, comments, shares, saved,
        ...
        const status = computeStatus(score, reach, avgReach);
